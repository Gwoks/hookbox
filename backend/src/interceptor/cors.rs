...
fn reflected_origin(req_headers: &BTreeMap<String, String>) -> String {
    req_headers.get("origin").cloned().unwrap_or_else(|| "*".to_string())
}
...
pub fn response_headers(req_headers: &BTreeMap<String, String>, cors_enabled: bool) -> Vec<(String, String)> {
    if !cors_enabled {
        return Vec::new();
    }
    vec![
        ("Access-Control-Allow-Origin".into(), reflected_origin(req_headers)),
...
pub fn preflight_headers(req_headers: &BTreeMap<String, String>, cors_enabled: bool) -> Vec<(String, String)> {
    if !cors_enabled {
        return Vec::new();
    }
...
mod tests {
    ...
    fn never_emits_allow_credentials() {
        ...
        for set in [response_headers(&h, true), preflight_headers(&h, true)] {
            assert!(!set.iter().any(|(k, _)| k.eq_ignore_ascii_case("access-control-allow-credentials")));
        }
    }

    #[test]
    fn reflects_origin_and_request_headers() {
        let h = headers(&[("origin", "https://x.io"), ("access-control-request-headers", "x-custom")]);
        let pf = preflight_headers(&h, true);
        assert!(pf.iter().any(|(k, v)| k == "Access-Control-Allow-Origin" && v == "https://x.io"));
        assert!(pf.iter().any(|(k, v)| k == "Access-Control-Allow-Headers" && v == "x-custom"));
        assert!(pf.iter().any(|(k, v)| k == "Access-Control-Allow-Methods" && v == ALLOW_METHODS));
    }
    ...
    fn falls_back_to_star_without_origin() {
        ...
        assert!(rh.iter().any(|(k, v)| k == "Access-Control-Allow-Origin" && v == "*"));
    }
