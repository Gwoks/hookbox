...
pub async fn handle_mock(
    ...
    if body.len() > state.cfg.max_ingest_body_bytes {
        let resp = json_response(413, &json!({"error":"payload_too_large","detail":"Request body exceeds the configured limit."}), "application/json");
    ...
    if method == "OPTIONS" {
        ...
        spawn_trace(state, token, &method, mock_path, 204, "cors", None, t0, 0, &headers_lower, &query, &body_text, &resp, &trace, &BTreeMap::new());
        return resp;
    }
    ...
    if ep.any_rule_gates_on_state {
        match crate::state_store::read_state(&state.pool, token).await {
            ...
            Err(_) => {
                ...
                trace.push(step("state_read", "store error -> state empty (fail-closed)"));
            }
        }
    }
    ...
    let selection = matcher::select(&ep.rules, &method, mock_path, &headers_lower, &query, &body_text, &endpoint_state);
    ...
    if let Some(m) = selection {
        ...
        trace.push(step("match", &format!("rule {} (priority {})", rule.id, rule.priority)));
        ...
        for w in &rule.state_writes {
            let value = templating::render(&w.value, &ctx, state.cfg.template_max_size, state.cfg.template_max_tags);
            match crate::state_store::write_state(&state.pool, token, &w.key, &value, state.cfg.state_ttl_seconds).await {
                Ok(true) => {
                    ...
                    state.feed_hub.publish(token, FeedEvent::new("state_changed", json!({"token": token, "key": w.key, "value": value})));
                }
                Ok(false) => trace.push(step("state_write", &format!("{}: unsafe key (skipped)", w.key))),
                Err(_) => trace.push(step("state_write", &format!("{}: store error (skipped)", w.key))),
            }
        }

        let body_out = templating::render(&rule.response.body_template, &ctx, state.cfg.template_max_size, state.cfg.template_max_tags);
        let status = rule.response.status_code.clamp(100, 599) as u16;
        let content_type = if rule.response.content_type.is_empty() { "application/json".to_string() } else { rule.response.content_type.clone() };
        resp = Response::builder().status(status).body(Body::from(body_out.clone())).unwrap();
        insert_header(resp.headers_mut(), "content-type", &content_type);
        for (k, v) in &rule.response.headers {
            insert_header(resp.headers_mut(), k, v);
        }
        trace.push(step("template", &format!("rendered {} bytes", body_out.len())));
    } else {
        // 4. No rule -> Auto-CRUD -> tunnel -> MITM -> default.
        let (r, sb) = resolve_unmatched(state, &ep, token, &method, mock_path, &query, &headers_lower, &body_text, &body, &mut trace).await;
        resp = r;
        served_by = sb;
    }
    ...
    let eff_chaos_mode = rule_chaos_override.clone().unwrap_or_else(|| ep.chaos_mode.clone());

    let eff_rate = conditions::clamp_rate(Some(eff_rate), &state.cfg);
    if eff_rate > 0 {
        let key = crate::limiter::Limiter::key(token, matched_rule_id.filter(|_| rule_rate_override.is_some()));
        let rl = state.limiter.check(&key, eff_rate, 60);
        if !rl.allowed {
            trace.push(step("rate_limit", &format!("429 (limit {eff_rate}/min)")));
            let mut r = json_response(429, &json!({"error":"rate_limited","detail":"Rate limit exceeded."}), "application/json");
            ...
            insert_header(r.headers_mut(), "x-ratelimit-remaining", &rl.remaining.to_string());
            let r = identified(r, token, "ratelimit", matched_rule_id, &ep, &headers_lower);
            spawn_trace(state, token, &method, mock_path, 429, "ratelimit", matched_rule_id, t0, 0, &headers_lower, &query, &body_text, &r, &trace, &endpoint_state);
            return r;
        }
    }
    ...
    match conditions::roll_chaos(chaos_pct, &eff_chaos_mode) {
        conditions::Chaos::Drop => {
            ...
            spawn_trace(state, token, &method, mock_path, 0, "chaos", matched_rule_id, t0, 0, &headers_lower, &query, &body_text, &Response::new(Body::empty()), &trace, &endpoint_state);
        ...
        conditions::Chaos::Status(code) => {
            ...
            let r = json_response(code as u16, &json!({"error":"chaos","detail":"Injected chaos failure."}), "application/json");
            let r = identified(r, token, "chaos", matched_rule_id, &ep, &headers_lower);
            spawn_trace(state, token, &method, mock_path, code, "chaos", matched_rule_id, t0, 0, &headers_lower, &query, &body_text, &r, &trace, &endpoint_state);
            return r;
        }
    ...
    let resp = identified(resp, token, &served_by, matched_rule_id, &ep, &headers_lower);

    // 7. Fire-and-forget trace + publish (never awaited).
    spawn_trace(state, token, &method, mock_path, status, &served_by, matched_rule_id, t0, applied_latency_ms, &headers_lower, &query, &body_text, &resp, &trace, &endpoint_state);
    resp
}
...
async fn resolve_unmatched(
        ...
        match crud::handle(state, token, method, mock_path, body_text).await {
            Ok(cr) => {
                ...
                let resp = match cr.body {
                    ...
                    None => Response::builder().status(cr.status).body(Body::empty()).unwrap(),
                };
                return (resp, "crud".into());
            }
            Err(_) => {
                trace.push(step("crud", "store error -> 503"));
                return (json_response(503, &json!({"error":"store_unavailable","detail":"Auto-CRUD store unavailable."}), "application/json"), "crud".into());
            }
        }
    }
    ...
    if state.tunnels.is_active(token) {
        match crate::routes::tunnel_ws::forward_to_tunnel(state, token, method, mock_path, query, headers_lower, body).await {
            Ok(resp) => {
            ...
            Err(_) => {
                ...
                return (json_response(504, &json!({"error":"no_tunnel","detail":"Tunnel error."}), "application/json"), "tunnel".into());
            }
        }
    }
    ...
    if let Some(target) = ep.target_url.as_deref() {
        if !target.trim().is_empty() {
            match proxy::forward(&state.cfg, target, method, mock_path, query, headers_lower, body).await {
                Ok(pr) => {
                    trace.push(step("forward", &format!("MITM -> {target}")));
                    let mut resp = Response::builder().status(pr.status).body(Body::from(pr.body)).unwrap();
                ...
                Err(proxy::ProxyError::Timeout) => {
                    ...
                    return (json_response(504, &json!({"error":"upstream_timeout","detail":"Upstream did not respond in time."}), "application/json"), "mitm".into());
                }
                Err(proxy::ProxyError::Unreachable(_)) => {
                    trace.push(step("forward", "MITM unreachable -> 502"));
                    return (json_response(502, &json!({"error":"upstream_unreachable","detail":"Could not reach the upstream target."}), "application/json"), "mitm".into());
                }
            }
        }
    }
    ...
    if ep.default_mode == "echo" {
        ...
        return (json_response(200, &payload, "application/json"), "default".into());
    }
    trace.push(step("default", "mock_404"));
    (json_response(404, &json!({"error":"no_match","detail":"No rule matched this request."}), "application/json"), "default".into())
}

// --- helpers -----------------------------------------------------------------

fn step(s: &str, detail: &str) -> Step {
    Step { step: s.to_string(), detail: detail.to_string() }
}
...
fn unknown_or_gone(token: &str, gone: bool) -> Response {
    let (status, code, detail) = if gone {
        (410, "endpoint_gone", "This endpoint was deleted or expired.")
    } else {
        (404, "unknown_endpoint", "No such endpoint.")
    };
    let mut r = json_response(status, &json!({"error": code, "detail": detail}), "application/json");
...
fn spawn_trace(
    ...
    let truncate = |s: &str| -> String {
        if s.len() > cap { s[..cap].to_string() } else { s.to_string() }
    };

    let trace_arr: Vec<Value> = trace.iter().map(|s| json!({"step": s.step, "detail": s.detail})).collect();
    let state_obj: serde_json::Map<String, Value> =
        state_snapshot.iter().map(|(k, v)| (k.clone(), json!(v))).collect();
    ...
    let rec = TraceRecord {
        ...
        request_headers: serde_json::to_string(&redact(req_headers)).unwrap_or_else(|_| "{}".into()),
        query_params: serde_json::to_string(query).unwrap_or_else(|_| "{}".into()),
        request_body: if req_body.is_empty() { None } else { Some(truncate(req_body)) },
...
fn jp(body: &str, path: &str) -> String {
    crate::helpers::jsonpath_lite(body, path).map(|s| s).unwrap_or_default()
}
