...
async fn run_interceptor(state: &AppState, result: &PlaneResult, req: Request<Body>) -> Response {
    ...
    if let Some(cl) = headers.get(axum::http::header::CONTENT_LENGTH).and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<usize>().ok()) {
        if cl > cap {
...
mod tests {
    ...
    async fn req(app: &Router, method: &str, host: &str, path: &str, bearer: Option<&str>, body: Option<Value>) -> (StatusCode, Value, Option<String>) {
        ...
        let plane = resp.headers().get("x-hookbox-plane").and_then(|v| v.to_str().ok()).map(String::from);
    ...
    async fn unauthed_api_is_401_session_is_open() {
        ...
        let (s2, _b2, _) = req(&app, "POST", "app.local", "/api/session", None, Some(json!({"email":"nope"}))).await;
    ...
    async fn endpoint_crud_round_trip() {
        ...
        // create
        let (s, body, _) = req(&app, "POST", "app.local", "/api/endpoints", Some(&secret), Some(json!({"name":"demo"}))).await;
        ...
        // list
        let (s, body, plane) = req(&app, "GET", "app.local", "/api/endpoints", Some(&secret), None).await;
        ...
        let (s, _b, _) = req(&app, "PATCH", "app.local", &format!("/api/endpoints/{token}"), Some(&secret),
            Some(json!({"target_url":"ftp://bad"}))).await;
        assert_eq!(s, StatusCode::UNPROCESSABLE_ENTITY);
        // delete -> tombstone, 200 Message
        let (s, body, _) = req(&app, "DELETE", "app.local", &format!("/api/endpoints/{token}"), Some(&secret), None).await;
        assert_eq!(s, StatusCode::OK);
    ...
    async fn rules_crud_and_204_and_order() {
        let (app, _pool, _oid, secret) = app_with_owner().await;
        let (_, ep, _) = req(&app, "POST", "app.local", "/api/endpoints", Some(&secret), Some(json!({}))).await;
        let token = ep["token"].as_str().unwrap().to_string();
        // create two rules with different priority
        let (s, r1, _) = req(&app, "POST", "app.local", &format!("/api/endpoints/{token}/rules"), Some(&secret),
            Some(json!({"priority": 50, "match": {"method":"GET"}}))).await;
        assert_eq!(s, StatusCode::CREATED);
        assert_eq!(r1["match"]["method"], json!("GET"));
        assert!(r1.as_object().unwrap().contains_key("webhook_action"));
        let (_, _r2, _) = req(&app, "POST", "app.local", &format!("/api/endpoints/{token}/rules"), Some(&secret),
            Some(json!({"priority": 10}))).await;
        // list ordered by priority,id
        let (s, list, _) = req(&app, "GET", "app.local", &format!("/api/endpoints/{token}/rules"), Some(&secret), None).await;
        assert_eq!(s, StatusCode::OK);
        ...
        // delete -> 204 no body
        let (s, body, _) = req(&app, "DELETE", "app.local", &format!("/api/endpoints/{token}/rules/{rid}"), Some(&secret), None).await;
        assert_eq!(s, StatusCode::NO_CONTENT);
        assert!(body.is_null());
        // get deleted -> 404
        let (s, _b, _) = req(&app, "GET", "app.local", &format!("/api/endpoints/{token}/rules/{rid}"), Some(&secret), None).await;
    ...
    async fn requests_limit_validation_and_state_collections() {
        ...
        let (s, _b, _) = req(&app, "GET", "app.local", &format!("/api/endpoints/{token}/requests?limit=500"), Some(&secret), None).await;
        assert_eq!(s, StatusCode::UNPROCESSABLE_ENTITY);
        // valid -> 200 empty list
        let (s, body, _) = req(&app, "GET", "app.local", &format!("/api/endpoints/{token}/requests?limit=10"), Some(&secret), None).await;
        assert_eq!(s, StatusCode::OK);
        assert_eq!(body.as_array().unwrap().len(), 0);
        // state peek -> {"state": {}}
        let (s, body, _) = req(&app, "GET", "app.local", &format!("/api/endpoints/{token}/state"), Some(&secret), None).await;
        assert_eq!(s, StatusCode::OK);
        assert_eq!(body["state"], json!({}));
        // collection unsafe name -> 422 invalid_collection
        let (s, body, _) = req(&app, "GET", "app.local", &format!("/api/endpoints/{token}/collections/bad%20name"), Some(&secret), None).await;
        assert_eq!(s, StatusCode::UNPROCESSABLE_ENTITY);
        assert_eq!(body["error"], json!("invalid_collection"));
        // valid collection -> {"items": []}
        let (s, body, _) = req(&app, "GET", "app.local", &format!("/api/endpoints/{token}/collections/widgets"), Some(&secret), None).await;
        assert_eq!(s, StatusCode::OK);
    ...
    async fn mock_host_api_never_reaches_management() {
        ...
        let (s, body, plane) = req(&app, "GET", "tok123.mock.local", "/api/endpoints", Some(&secret), None).await;
        assert_eq!(s, StatusCode::NOT_FOUND);
    ...
    async fn mock_host_served_by_rule_and_default_404() {
        ...
        sqlx::query("INSERT INTO endpoints (token, owner_id) VALUES ('tokLIVE001', ?)")
            .bind(&oid).execute(&pool).await.unwrap();
        sqlx::query(
        ...
        let request = Request::builder()
            .method("GET").uri("/hello").header("host", "tokLIVE001.mock.local")
            .body(Body::empty()).unwrap();
        let resp = app.clone().oneshot(request).await.unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED);
        assert_eq!(resp.headers().get("x-hookbox-served-by").unwrap(), "rule");
        assert_eq!(resp.headers().get("x-hookbox-endpoint").unwrap(), "tokLIVE001");
        assert_eq!(resp.headers().get("x-hookbox-rule-id").unwrap(), "1");

        // Non-matching path -> default mock_404.
        let request = Request::builder()
            .method("GET").uri("/nope").header("host", "tokLIVE001.mock.local")
            .body(Body::empty()).unwrap();
        let resp = app.clone().oneshot(request).await.unwrap();
        assert_eq!(resp.status(), StatusCode::NOT_FOUND);
        assert_eq!(resp.headers().get("x-hookbox-served-by").unwrap(), "default");
    }
    ...
    async fn non_owner_endpoint_is_404() {
        ...
        sqlx::query("INSERT INTO owners (owner_id, email, secret_hash) VALUES (?,?,?)")
            .bind(&b_id).bind("b@c.com").bind(hash_secret("xyz")).execute(&pool).await.unwrap();
        sqlx::query("INSERT INTO endpoints (token, owner_id) VALUES (?,?)")
            .bind("tokBBBBBBB").bind(&b_id).execute(&pool).await.unwrap();
        // owner A (secret) cannot see it -> 404 not 403.
        let (s, body, _) = req(&app, "GET", "app.local", "/api/endpoints/tokBBBBBBB", Some(&secret), None).await;
        assert_eq!(s, StatusCode::NOT_FOUND);
    ...
    async fn session_creates_owner_and_primary_endpoint() {
        let (app, _pool, _oid, _secret) = app_with_owner().await;
        let (s, body, plane) = req(&app, "POST", "app.local", "/api/session", None, Some(json!({"email":"new@user.com"}))).await;
        assert_eq!(s, StatusCode::OK);
        assert_eq!(plane.as_deref(), Some("api"));
