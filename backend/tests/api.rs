...
async fn app() -> (axum::Router, String) {
    ...
    let (_s, body, _) = call(&app, "POST", "app.local", "/api/session", None, Some(json!({"email":"t@e.com"}))).await;
...
async fn call(
    ...
    let req = match body {
        Some(j) => b.header("content-type", "application/json").body(Body::from(serde_json::to_vec(&j).unwrap())).unwrap(),
...
async fn new_endpoint(app: &axum::Router, secret: &str) -> String {
    let (s, body, _) = call(app, "POST", "app.local", "/api/endpoints", Some(secret), Some(json!({"name":"e"}))).await;
    assert_eq!(s, StatusCode::CREATED);
    body["token"].as_str().unwrap().to_string()
}

async fn add_rule(app: &axum::Router, secret: &str, token: &str, rule: Value) -> i64 {
    let (s, body, _) = call(app, "POST", "app.local", &format!("/api/endpoints/{token}/rules"), Some(secret), Some(rule)).await;
...
async fn session_shape_and_flat_error_envelope() {
    ...
    // bad email -> 422.
    let (s, _b, _) = call(&app, "POST", "app.local", "/api/session", None, Some(json!({"email":"nope"}))).await;
...
async fn rules_ordered_and_204_delete() {
    ...
    let r1 = add_rule(&app, &secret, &token, json!({"priority": 50, "match": {"method":"GET"}})).await;
    add_rule(&app, &secret, &token, json!({"priority": 10})).await;
    let (s, list, _) = call(&app, "GET", "app.local", &format!("/api/endpoints/{token}/rules"), Some(&secret), None).await;
    assert_eq!(s, StatusCode::OK);
    let arr = list.as_array().unwrap();
    assert_eq!(arr[0]["priority"], json!(10)); // priority,id order
    // DELETE -> 204 no body.
    let (s, body, _) = call(&app, "DELETE", "app.local", &format!("/api/endpoints/{token}/rules/{r1}"), Some(&secret), None).await;
...
async fn unknown_404_and_gone_410_not_traced() {
    ...
    let token = new_endpoint(&app, &secret).await;
    call(&app, "DELETE", "app.local", &format!("/api/endpoints/{token}"), Some(&secret), None).await;
    let host = format!("{token}.mock.local");
...
async fn default_modes_404_and_echo() {
    ...
    // echo mode.
    call(&app, "PATCH", "app.local", &format!("/api/endpoints/{token}"), Some(&secret), Some(json!({"default_mode":"echo"}))).await;
    let (s, body, _) = call(&app, "POST", &host, "/echo/me?q=1", None, Some(json!({"a":1}))).await;
    assert_eq!(s, StatusCode::OK);
...
async fn auto_crud_lifecycle() {
    let (app, secret) = app().await;
    let token = new_endpoint(&app, &secret).await;
    call(&app, "PATCH", "app.local", &format!("/api/endpoints/{token}"), Some(&secret), Some(json!({"auto_crud": true}))).await;
    let host = format!("{token}.mock.local");

    // POST -> 201 with server uuid id.
    let (s, body, _) = call(&app, "POST", &host, "/books", None, Some(json!({"title":"Dune"}))).await;
    assert_eq!(s, StatusCode::CREATED);
    ...
    assert_eq!(s, StatusCode::OK);
    let (s, _b, _) = call(&app, "PUT", &host, &format!("/books/{id}"), None, Some(json!({"title":"Foundation"}))).await;
    assert_eq!(s, StatusCode::OK);
    let (s, _b, _) = call(&app, "PATCH", &host, &format!("/books/{id}"), None, Some(json!({"year":1951}))).await;
    assert_eq!(s, StatusCode::OK);
...
async fn cors_preflight_and_response_headers() {
    ...
    let req = Request::builder()
        .method("OPTIONS").uri("/anything").header("host", &host)
    ...
    let req = Request::builder().method("GET").uri("/x").header("host", &host).header("origin", "https://x.io").body(Body::empty()).unwrap();
    let resp = app.clone().oneshot(req).await.unwrap();
    assert_eq!(resp.headers().get("access-control-allow-origin").unwrap(), "https://x.io");

    // P2 (/api) emits NO wildcard CORS.
    let (_s, _b, h) = call(&app, "GET", "app.local", "/api/endpoints", Some(&secret), None).await;
...
async fn rate_limit_429_with_headers() {
    let (app, secret) = app().await;
    let token = new_endpoint(&app, &secret).await;
    call(&app, "PATCH", "app.local", &format!("/api/endpoints/{token}"), Some(&secret), Some(json!({"rate_limit_per_min": 2}))).await;
    let host = format!("{token}.mock.local");
...
async fn chaos_100pct_always_fires_5xx() {
    let (app, secret) = app().await;
    let token = new_endpoint(&app, &secret).await;
    call(&app, "PATCH", "app.local", &format!("/api/endpoints/{token}"), Some(&secret), Some(json!({"chaos_pct": 100}))).await;
    let host = format!("{token}.mock.local");
    let (s, body, h) = call(&app, "GET", &host, "/x", None, None).await;
...
async fn templating_ssti_probes_are_literal() {
    ...
    let host = format!("{token}.mock.local");
    let req = Request::builder().method("GET").uri("/t").header("host", &host).body(Body::empty()).unwrap();
    let resp = app.clone().oneshot(req).await.unwrap();
    let bytes = resp.into_body().collect().await.unwrap().to_bytes();
...
async fn state_write_then_gated_rule() {
    ...
    let req = Request::builder().method("POST").uri("/login").header("host", &host).body(Body::empty()).unwrap();
    let resp = app.clone().oneshot(req).await.unwrap();
    let bytes = resp.into_body().collect().await.unwrap().to_bytes();
    ...
    let (s, body, _) = call(&app, "GET", "app.local", &format!("/api/endpoints/{token}/state"), Some(&secret), None).await;
    assert_eq!(s, StatusCode::OK);
...
async fn served_requests_are_traced_with_redaction() {
    let (app, secret) = app().await;
    let token = new_endpoint(&app, &secret).await;
    add_rule(&app, &secret, &token, json!({"match":{"method":"GET","path":"/p"},"response":{"status_code":200}})).await;
    let host = format!("{token}.mock.local");
    // include an Authorization header that must be redacted in the stored trace.
    let req = Request::builder().method("GET").uri("/p").header("host", &host).header("authorization", "Bearer leak").body(Body::empty()).unwrap();
    let _ = app.clone().oneshot(req).await.unwrap();

    // Give the spawned trace task a moment.
    tokio::time::sleep(std::time::Duration::from_millis(80)).await;

    let (s, list, _) = call(&app, "GET", "app.local", &format!("/api/endpoints/{token}/requests"), Some(&secret), None).await;
    assert_eq!(s, StatusCode::OK);
    let arr = list.as_array().unwrap();
    assert!(!arr.is_empty(), "served request should be traced");
    let id = arr[0]["id"].as_i64().unwrap();
    let (s, detail, _) = call(&app, "GET", "app.local", &format!("/api/requests/{id}"), Some(&secret), None).await;
    assert_eq!(s, StatusCode::OK);
    // owner cap redacted before persist.
    assert_eq!(detail["request_headers"]["authorization"], json!("<redacted>"));
